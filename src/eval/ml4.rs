// src/ml4.rs

use crate::ast::{Expr, Value, Env, Var, Op};
use std::fmt;
use std::rc::Rc;
pub struct Derivation {
    pub env: Env,
    pub expr: Expr,
    pub result: Value,
    pub rule: String,
    pub sub_derivations: Vec<Derivation>,
}

impl Derivation {
        fn fmt_with_indent(&self, f: &mut fmt::Formatter<'_>, indent_level: usize) -> fmt::Result {
        let indent_str = "    ".repeat(indent_level);
        writeln!(
            f,
            "{}{} {} evalto {} by {} {{",
            indent_str,
            format_env(&self.env),
            self.expr.to_ml4_string(),
            self.result.to_ml4_string(),
            self.rule
        )?;
        for sub in &self.sub_derivations {
            sub.fmt_with_indent(f, indent_level + 1)?;
        }
        writeln!(f, "{}}}", indent_str)
    }
}

impl fmt::Display for Derivation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.fmt_with_indent(f, 0)
    }
}

fn format_env(env: &Env) -> String {
    if env.is_empty() {
        String::from("|-")
    } else {
        let binds = env
            .iter()
            .map(|(v, val)| format!("{} = {}", v.0, val.to_ml4_string()))
            .collect::<Vec<_>>()
            .join(", ");
        format!("{} |-", binds)
    }
}

pub trait ToML4String {
    fn to_ml4_string(&self) -> String;
}

impl ToML4String for Expr {
    fn to_ml4_string(&self) -> String {
        match self {
            Expr::Int(i) => i.to_string(),
            Expr::Bool(b) => b.to_string(),
            Expr::Var(v) => v.0.clone(),
            Expr::Let(v, e1, e2) => format!("let {} = {} in {}", v.0, e1.to_ml4_string(), e2.to_ml4_string()),
            Expr::LetRec(f, x, body, e2) => format!("let rec {} = fun {} -> {} in {}", f.0, x.0, body.to_ml4_string(), e2.to_ml4_string()),
            Expr::If(c, t, e) => format!("if {} then {} else {}", c.to_ml4_string(), t.to_ml4_string(), e.to_ml4_string()),
            Expr::BinOp(e1, op, e2) => format!("{} {} {}", e1.to_ml4_string(), op.to_ml4_string(), e2.to_ml4_string()),
            Expr::Fun(param, body) => format!("fun {} -> {}", param.0, body.to_ml4_string()),
            Expr::App(f, arg) => format!("{} {}", f.to_ml4_string(), arg.to_ml4_string()),
            Expr::Nil => "[]".to_string(),
            Expr::Cons(h, t) => format!("{} :: {}", h.to_ml4_string(), t.to_ml4_string()),
            Expr::Match(e, nil_case, hd, tl, cons_case) => format!(
                "match {} with [] -> {} | {} :: {} -> {}",
                e.to_ml4_string(),
                nil_case.to_ml4_string(),
                hd.0,
                tl.0,
                cons_case.to_ml4_string()
            ),
        }
    }
}

impl ToML4String for Value {
    fn to_ml4_string(&self) -> String {
        match self {
            Value::Int(i) => i.to_string(),
            Value::Bool(b) => b.to_string(),
            Value::Nil => "[]".to_string(),
            Value::Cons(h, t) => format!("{} :: {}", h.to_ml4_string(), t.to_ml4_string()),
            Value::FunVal(param, body, env) => {
                let env_str = if env.is_empty() {
                    "()".to_string()
                } else {
                    format!("({})", env.iter().map(|(v, val)| format!("{} = {}", v.0, val.to_ml4_string())).collect::<Vec<_>>().join(", "))
                };
                format!("{}[fun {} -> {}]", env_str, param.0, body.to_ml4_string())
            }
            Value::RecFunVal(f, x, body, env) => {
                let env_str = if env.is_empty() {
                    "()".to_string()
                } else {
                    format!("({})", env.iter().map(|(v, val)| format!("{} = {}", v.0, val.to_ml4_string())).collect::<Vec<_>>().join(", "))
                };
                format!("{}[rec {} = fun {} -> {}]", env_str, f.0, x.0, body.to_ml4_string())
            }
        }
    }
}

impl ToML4String for Op {
    fn to_ml4_string(&self) -> String {
        match self {
            Op::Add => "+",
            Op::Sub => "-",
            Op::Mul => "*",
            Op::Lt => "<",
        }
        .to_string()
    }
}

pub fn derive(env: &Env, expr: &Expr) -> Derivation {
    match expr {
        Expr::Int(i) => Derivation {
            env: Rc::clone(env),
            expr: expr.clone(),
            result: Value::Int(*i),
            rule: "E-Int".to_string(),
            sub_derivations: vec![],
        },
        Expr::Bool(b) => Derivation {
            env: Rc::clone(env),
            expr: expr.clone(),
            result: Value::Bool(*b),
            rule: "E-Bool".to_string(),
            sub_derivations: vec![],
        },
        Expr::Var(x) => {
            for (v, val) in env.iter().rev() {
                if v == x {
                    return Derivation {
                        env: Rc::clone(env),
                        expr: expr.clone(),
                        result: val.clone(),
                        rule: "E-Var".to_string(),
                        sub_derivations: vec![],
                    };
                }
            }
            panic!("Unbound variable: {}", x.0)
        }
        Expr::BinOp(e1, op, e2) => {
            let d1 = derive(env, e1);
            let d2 = derive(env, e2);
            let result = match (d1.result.clone(), d2.result.clone(), op) {
                (Value::Int(i1), Value::Int(i2), Op::Add) => Value::Int(i1 + i2),
                (Value::Int(i1), Value::Int(i2), Op::Sub) => Value::Int(i1 - i2),
                (Value::Int(i1), Value::Int(i2), Op::Mul) => Value::Int(i1 * i2),
                (Value::Int(i1), Value::Int(i2), Op::Lt) => Value::Bool(i1 < i2),
                _ => panic!("Invalid binary op eval"),
            };
            let rule = match op {
                Op::Add => "E-Plus",
                Op::Sub => "E-Minus",
                Op::Mul => "E-Times",
                Op::Lt => "E-Lt",
                _ => unreachable!(),
            };
            Derivation {
                env: Rc::clone(env),
                expr: expr.clone(),
                result,
                rule: rule.to_string(),
                sub_derivations: vec![d1, d2],
            }
        }
        Expr::If(cond, e_then, e_else) => {
            let d_cond = derive(env, cond);
            match d_cond.result {
                Value::Bool(true) => {
                    let d_then = derive(env, e_then);
                    Derivation {
                        env: Rc::clone(env),
                        expr: expr.clone(),
                        result: d_then.result.clone(),
                        rule: "E-IfT".to_string(),
                        sub_derivations: vec![d_cond, d_then],
                    }
                }
                Value::Bool(false) => {
                    let d_else = derive(env, e_else);
                    Derivation {
                        env: Rc::clone(env),
                        expr: expr.clone(),
                        result: d_else.result.clone(),
                        rule: "E-IfF".to_string(),
                        sub_derivations: vec![d_cond, d_else],
                    }
                }
                _ => panic!("Condition must evaluate to a boolean"),
            }
        }
        Expr::Let(x, e1, e2) => {
            let d1 = derive(env, e1);
            let mut new_env = (**env).clone();
            new_env.push((x.clone(), d1.result.clone()));
            let rc_env = Rc::new(new_env);
            let d2 = derive(&rc_env, e2);
            Derivation {
                env: Rc::clone(env),
                expr: expr.clone(),
                result: d2.result.clone(),
                rule: "E-Let".to_string(),
                sub_derivations: vec![d1, d2],
            }
        }
        Expr::Fun(param, body) => {
            Derivation {
                env: Rc::clone(env),
                expr: expr.clone(),
                result: Value::FunVal(param.clone(), body.clone(), Rc::clone(env)),
                rule: "E-Fun".to_string(),
                sub_derivations: vec![],
            }
        }
        Expr::App(f, arg) => {
            let df = derive(env, f);
            let darg = derive(env, arg);
            let result;
            let mut sub_derivations;
            match &df.result {
                Value::FunVal(param, body, closure_env) => {
                    let mut new_env = (**closure_env).to_vec();
                    new_env.push((param.clone(), darg.result.clone()));
                    let rc_env = Rc::new(new_env);
                    let d_body = derive(&rc_env, body);
                    result = d_body.result.clone();
                    sub_derivations = vec![df, darg, d_body];
                    Derivation {
                        env: Rc::clone(env),
                        expr: expr.clone(),
                        result,
                        rule: "E-App".to_string(),
                        sub_derivations,
                    }
                }
                Value::RecFunVal(name, param, body, closure_env) => {
                    let mut new_env = (**closure_env).to_vec();
                    new_env.push((param.clone(), darg.result.clone()));
                    new_env.push((name.clone(), Value::RecFunVal(name.clone(), param.clone(), body.clone(), Rc::new(new_env.clone()))));
                    let rc_env = Rc::new(new_env);
                    let d_body = derive(&rc_env, body);
                    result = d_body.result.clone();
                    sub_derivations = vec![df, darg, d_body];
                    Derivation {
                        env: Rc::clone(env),
                        expr: expr.clone(),
                        result,
                        rule: "E-AppRec".to_string(),
                        sub_derivations,
                    }
                }
                _ => panic!("Tried to apply non-function"),
            }
        }
        Expr::LetRec(f, x, body, e2) => {
            let mut new_env = (**env).clone();
            let rec_val = Value::RecFunVal(f.clone(), x.clone(), body.clone(), Rc::new(new_env.clone()));
            new_env.push((f.clone(), rec_val.clone()));
            let rc_env = Rc::new(new_env);
            let d2 = derive(&rc_env, e2);
            Derivation {
                env: Rc::clone(env),
                expr: expr.clone(),
                result: d2.result.clone(),
                rule: "E-LetRec".to_string(),
                sub_derivations: vec![d2],
            }
        }
        Expr::Nil => Derivation {
            env: Rc::clone(env),
            expr: expr.clone(),
            result: Value::Nil,
            rule: "E-Nil".to_string(),
            sub_derivations: vec![],
        },

        Expr::Cons(e1, e2) => {
            let d1 = derive(env, e1);
            let d2 = derive(env, e2);
            Derivation {
                env: Rc::clone(env),
                expr: expr.clone(),
                result: Value::Cons(Box::new(d1.result.clone()), Box::new(d2.result.clone())),
                rule: "E-Cons".to_string(),
                sub_derivations: vec![d1, d2],
            }
        },

        Expr::Match(e, e_nil, x, y, e_cons) => {
            let d_expr = derive(env, e);
            match d_expr.result.clone() {
                Value::Nil => {
                    let d_nil = derive(env, e_nil);
                    Derivation {
                        env: Rc::clone(env),
                        expr: expr.clone(),
                        result: d_nil.result.clone(),
                        rule: "E-MatchNil".to_string(),
                        sub_derivations: vec![d_expr, d_nil],
                    }
                }
                Value::Cons(v1, v2) => {
                    let mut new_env = (**env).clone();
                    new_env.push((x.clone(), *v1));
                    new_env.push((y.clone(), *v2));
                    let rc_env = Rc::new(new_env);
                    let d_cons = derive(&rc_env, e_cons);
                    Derivation {
                        env: Rc::clone(env),
                        expr: expr.clone(),
                        result: d_cons.result.clone(),
                        rule: "E-MatchCons".to_string(),
                        sub_derivations: vec![d_expr, d_cons],
                    }
                }
                _ => panic!("Cannot match on non-list value"),
            }
        }
        _ => unimplemented!("Derivation not yet handled for this expression form")
    }
} 
